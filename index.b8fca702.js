...
class Game {
    ...
    moveRight() {
        if (this.status !== "playing") return;
        const newState = this.state.map((row)=>this.compressAndMerge([
                ...row
            ].reverse()).reverse());
        if (!this.equals(newState, this.state)) {
            this.state = newState;
    ...
    moveDown() {
        if (this.status !== "playing") return;
        const transposed = this.transpose(this.state);
        const newTransposed = transposed.map((col)=>this.compressAndMerge([
                ...col
            ].reverse()).reverse());
        const newState = this.transpose(newTransposed);
        if (!this.equals(newState, this.state)) {
    ...
    updateStatus() {
        ...
        if (this.hasMoves()) this.status = "playing";
    ...
    hasMoves() {
        ...
        for(let r = 0; r < 4; r++)for(let c = 0; c < 4; c++){
            ...
            if (r < 3 && this.state[r + 1][c] === val) return true;
            if (c < 3 && this.state[r][c + 1] === val) return true;
        }
        return false;
    }
}
